# OBJC-HomemadeBlockProgram — Afrikaans

Dit is ’n Objective-C-opdraglynprogram vir OpenBSD wat aanvaller-IP’s uit logs blokkeer en ou blokkasies laat verval.

Gelokaliseerde gebruiksaanwysings is hieronder; die volledige Engelse README is ingesluit vir volle pariteit.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Blokkeer IP’s wat in sshd "Invalid user"-loginskrywings verskyn.
- `pf-blocker --monitor-disconnect`  
  Blokkeer IP’s wat in sshd "Received disconnect from"-loginskrywings verskyn.
- `pf-blocker --monitor-allowlist-violations`  
  Blokkeer IP’s wat die web-allowlist-oortredingsdrempel binne die ingestelde venster oorskry.
- `pf-blocker --monitor-slowloris-violations`  
  Voer IP’s wat deur die Slowloris-detektor gemerk is in die blocker-ledger in.
- `pf-blocker --monitor-ddos`  
  Voer IP’s wat deur die DDoS-detektor gemerk is in die blocker-ledger in.
- `pf-blocker --expire-blocks`  
  Verwyder ou blokkades uit die bloklêer en ledger.
