# OBJC-HomemadeBlockProgram — العربية

برنامج سطر أوامر بلغة Objective-C لنظام OpenBSD يقوم بحظر عناوين IP المهاجمة من السجلات وإزالة الحظر القديم.

توجد إرشادات استخدام مترجمة أدناه؛ وتم تضمين README الإنجليزي الكامل لتحقيق التكافؤ الكامل.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  يحظر عناوين IP الظاهرة في سجلات sshd "Invalid user".
- `pf-blocker --monitor-disconnect`  
  يحظر عناوين IP الظاهرة في سجلات sshd "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  يحظر عناوين IP التي تتجاوز حد مخالفات قائمة السماح ضمن النافذة المحددة.
- `pf-blocker --monitor-slowloris-violations`  
  يضيف عناوين IP المعلَّمة بواسطة كاشف Slowloris إلى سجل الحظر.
- `pf-blocker --monitor-ddos`  
  يضيف عناوين IP المعلَّمة بواسطة كاشف DDoS إلى سجل الحظر.
- `pf-blocker --expire-blocks`  
  يزيل الحظر القديم من ملف الحظر والسجل.
