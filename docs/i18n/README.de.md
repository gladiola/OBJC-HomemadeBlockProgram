# OBJC-HomemadeBlockProgram — Deutsch

Ein Objective-C-Kommandozeilenprogramm für OpenBSD, das Angreifer-IP-Adressen aus Logs blockiert und alte Sperren entfernt.

Lokalisierte Nutzungsanweisungen stehen unten; das vollständige englische README ist zur vollständigen Parität enthalten.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Blockiert IPs aus sshd-Logeinträgen „Invalid user“.
- `pf-blocker --monitor-disconnect`  
  Blockiert IPs aus sshd-Logeinträgen „Received disconnect from“.
- `pf-blocker --monitor-allowlist-violations`  
  Blockiert IPs, die den Schwellwert für Allowlist-Verstöße im Zeitfenster überschreiten.
- `pf-blocker --monitor-slowloris-violations`  
  Übernimmt vom Slowloris-Detektor markierte IPs in das Blocker-Ledger.
- `pf-blocker --monitor-ddos`  
  Übernimmt vom DDoS-Detektor markierte IPs in das Blocker-Ledger.
- `pf-blocker --expire-blocks`  
  Entfernt alte Sperren aus Blockdatei und Ledger.
