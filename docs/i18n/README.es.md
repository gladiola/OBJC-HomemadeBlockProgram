# OBJC-HomemadeBlockProgram — Español

Un programa de línea de comandos en Objective-C para OpenBSD que bloquea IPs atacantes desde registros y caduca bloqueos antiguos.

Las instrucciones de uso localizadas están abajo; se incluye el README completo en inglés para paridad total.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Bloquea IPs vistas en entradas de sshd "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Bloquea IPs vistas en entradas de sshd "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Bloquea IPs que superan el umbral de violaciones de lista permitida dentro de la ventana configurada.
- `pf-blocker --monitor-slowloris-violations`  
  Incorpora al ledger las IP marcadas por el detector Slowloris.
- `pf-blocker --monitor-ddos`  
  Incorpora al ledger las IP marcadas por el detector DDoS.
- `pf-blocker --expire-blocks`  
  Elimina bloqueos antiguos del archivo de bloqueos y del ledger.
