# OBJC-HomemadeBlockProgram — Suomi

Tämä on OpenBSD:lle tehty Objective-C-komentoriviohjelma, joka estää hyökkääjien IP-osoitteet lokeista ja vanhentaa vanhat estot.

Paikalliset käyttöohjeet ovat alla; täydellinen englanninkielinen README on mukana täyden vastaavuuden vuoksi.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Estä IP-osoitteet, jotka näkyvät sshd:n "Invalid user" -lokiriveillä.
- `pf-blocker --monitor-disconnect`  
  Estä IP-osoitteet, jotka näkyvät sshd:n "Received disconnect from" -lokiriveillä.
- `pf-blocker --monitor-allowlist-violations`  
  Estä IP-osoitteet, jotka ylittävät web-allowlist-rikkomuskynnyksen määritetyssä ikkunassa.
- `pf-blocker --monitor-slowloris-violations`  
  Tuo Slowloris-havaitsimen merkitsemät IP-osoitteet blocker-ledgeriin.
- `pf-blocker --monitor-ddos`  
  Tuo DDoS-havaitsimen merkitsemät IP-osoitteet blocker-ledgeriin.
- `pf-blocker --expire-blocks`  
  Poista vanhat estot estotiedostosta ja ledgeristä.
