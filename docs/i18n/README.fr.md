# OBJC-HomemadeBlockProgram — Français

Un programme en ligne de commande Objective-C pour OpenBSD qui bloque les IP attaquantes depuis les journaux et expire les anciens blocages.

Des instructions d’utilisation localisées figurent ci-dessous ; le README anglais complet est inclus pour une parité totale.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Bloque les IP vues dans les entrées sshd « Invalid user ».
- `pf-blocker --monitor-disconnect`  
  Bloque les IP vues dans les entrées sshd « Received disconnect from ».
- `pf-blocker --monitor-allowlist-violations`  
  Bloque les IP qui dépassent le seuil de violations de la liste autorisée dans la fenêtre configurée.
- `pf-blocker --monitor-slowloris-violations`  
  Ajoute au registre les IP signalées par le détecteur Slowloris.
- `pf-blocker --monitor-ddos`  
  Ajoute au registre les IP signalées par le détecteur DDoS.
- `pf-blocker --expire-blocks`  
  Supprime les anciens blocages du fichier de blocage et du registre.
