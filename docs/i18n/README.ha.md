# OBJC-HomemadeBlockProgram — Hausa

Wannan shiri ne na layin umarni na Objective-C don OpenBSD wanda ke toshe IP na maharan daga log kuma yana karewa tsoffin toshewa.

An bayar da umarnin amfani da aka fassara a ƙasa; an haɗa cikakken README na Turanci don cikakkiyar daidaito.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Toshe IP da aka gani a rubutun sshd "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Toshe IP da aka gani a rubutun sshd "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Toshe IP da suka wuce iyakar karya allowlist a cikin lokacin da aka saita.
- `pf-blocker --monitor-slowloris-violations`  
  Shigar da IP da Slowloris detector ya yiwa alama cikin ledger na blocker.
- `pf-blocker --monitor-ddos`  
  Shigar da IP da DDoS detector ya yiwa alama cikin ledger na blocker.
- `pf-blocker --expire-blocks`  
  Cire tsoffin toshewa daga fayil na toshewa da ledger.
