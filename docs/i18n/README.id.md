# OBJC-HomemadeBlockProgram — Bahasa Indonesia

Ini adalah program baris perintah Objective-C untuk OpenBSD yang memblokir IP penyerang dari log dan menghapus blokir lama yang kedaluwarsa.

Petunjuk penggunaan terlokalisasi ada di bawah; README bahasa Inggris lengkap disertakan untuk kesetaraan penuh.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Blokir IP yang terlihat pada entri log sshd "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Blokir IP yang terlihat pada entri log sshd "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Blokir IP yang melebihi ambang pelanggaran allowlist web dalam jendela yang dikonfigurasi.
- `pf-blocker --monitor-slowloris-violations`  
  Impor IP yang ditandai detektor Slowloris ke ledger blocker.
- `pf-blocker --monitor-ddos`  
  Impor IP yang ditandai detektor DDoS ke ledger blocker.
- `pf-blocker --expire-blocks`  
  Hapus blok lama dari file blok dan ledger.
