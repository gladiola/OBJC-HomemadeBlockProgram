# OBJC-HomemadeBlockProgram — Italiano

Un programma a riga di comando Objective-C per OpenBSD che blocca IP di attaccanti dai log e rimuove i blocchi scaduti.

Le istruzioni d’uso localizzate sono sotto; il README completo in inglese è incluso per la piena parità.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Blocca IP visti nelle voci sshd "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Blocca IP visti nelle voci sshd "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Blocca IP che superano la soglia di violazioni della allowlist nella finestra configurata.
- `pf-blocker --monitor-slowloris-violations`  
  Importa nel registro gli IP segnalati dal rilevatore Slowloris.
- `pf-blocker --monitor-ddos`  
  Importa nel registro gli IP segnalati dal rilevatore DDoS.
- `pf-blocker --expire-blocks`  
  Rimuove i blocchi vecchi dal file blocchi e dal registro.
