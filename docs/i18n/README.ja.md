# OBJC-HomemadeBlockProgram — 日本語

これは OpenBSD 向けの Objective-C コマンドラインプログラムで、ログから攻撃元 IP をブロックし、古いブロックを期限切れにします。

以下にローカライズされた使用手順を示し、完全な同等性のため英語 README 全文を含めています。

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  sshd の「Invalid user」ログに現れた IP をブロックします。
- `pf-blocker --monitor-disconnect`  
  sshd の「Received disconnect from」ログに現れた IP をブロックします。
- `pf-blocker --monitor-allowlist-violations`  
  設定された時間枠内で Web allowlist 違反しきい値を超えた IP をブロックします。
- `pf-blocker --monitor-slowloris-violations`  
  Slowloris 検知器がフラグした IP をブロッカ台帳に取り込みます。
- `pf-blocker --monitor-ddos`  
  DDoS 検知器がフラグした IP をブロッカ台帳に取り込みます。
- `pf-blocker --expire-blocks`  
  ブロックファイルと台帳から古いブロックを削除します。
