# OBJC-HomemadeBlockProgram — Basa Jawa

Iki program baris-perintah Objective-C kanggo OpenBSD sing mblokir IP panyerang saka log lan ngentekake blok lawas.

Pandhuan panggunaan sing wis dilokalisasi ana ing ngisor; README Inggris lengkap kalebu kanggo paritas lengkap.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Blokir IP sing katon ing log sshd "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Blokir IP sing katon ing log sshd "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Blokir IP sing ngluwihi ambang pelanggaran allowlist web ing jendhela sing disetel.
- `pf-blocker --monitor-slowloris-violations`  
  Lebokna IP sing ditandai detektor Slowloris menyang ledger blocker.
- `pf-blocker --monitor-ddos`  
  Lebokna IP sing ditandai detektor DDoS menyang ledger blocker.
- `pf-blocker --expire-blocks`  
  Busak blok lawas saka file blok lan ledger.
