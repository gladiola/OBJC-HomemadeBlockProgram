# OBJC-HomemadeBlockProgram — Nederlands

Dit is een Objective-C commandoregelprogramma voor OpenBSD dat aanvallers-IP’s uit logs blokkeert en oude blokkades laat verlopen.

Gelokaliseerde gebruiksinstructies staan hieronder; de volledige Engelse README is opgenomen voor volledige pariteit.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Blokkeer IP’s die in sshd-logs met "Invalid user" voorkomen.
- `pf-blocker --monitor-disconnect`  
  Blokkeer IP’s die in sshd-logs met "Received disconnect from" voorkomen.
- `pf-blocker --monitor-allowlist-violations`  
  Blokkeer IP’s die binnen het ingestelde venster de drempel voor allowlist-overtredingen overschrijden.
- `pf-blocker --monitor-slowloris-violations`  
  Voeg door de Slowloris-detector gemarkeerde IP’s toe aan het blocker-ledger.
- `pf-blocker --monitor-ddos`  
  Voeg door de DDoS-detector gemarkeerde IP’s toe aan het blocker-ledger.
- `pf-blocker --expire-blocks`  
  Verwijder oude blokkades uit blokbestand en ledger.
