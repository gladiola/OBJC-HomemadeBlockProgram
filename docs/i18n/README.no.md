# OBJC-HomemadeBlockProgram — Norsk

Dette er et Objective-C-kommandolinjeprogram for OpenBSD som blokkerer angriper-IP fra logger og utløper gamle blokkeringer.

Lokaliserte bruksinstruksjoner er nedenfor; hele den engelske README-en er inkludert for full paritet.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Blokker IP-er som finnes i sshd-logg med "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Blokker IP-er som finnes i sshd-logg med "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Blokker IP-er som overskrider terskelen for allowlist-brudd innenfor konfigurert vindu.
- `pf-blocker --monitor-slowloris-violations`  
  Importer IP-er flagget av Slowloris-detektoren til blocker-ledgeren.
- `pf-blocker --monitor-ddos`  
  Importer IP-er flagget av DDoS-detektoren til blocker-ledgeren.
- `pf-blocker --expire-blocks`  
  Fjern gamle blokkeringer fra blokkfilen og ledgeren.
