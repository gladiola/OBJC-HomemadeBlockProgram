# OBJC-HomemadeBlockProgram — Svenska

Detta är ett Objective-C-kommandoradsprogram för OpenBSD som blockerar angripar-IP från loggar och låter gamla blockeringar löpa ut.

Lokaliserade användningsinstruktioner finns nedan; hela den engelska README:n ingår för full paritet.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Blockera IP-adresser som syns i sshd-loggar med "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Blockera IP-adresser som syns i sshd-loggar med "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Blockera IP-adresser som överskrider tröskeln för allowlist-överträdelser inom konfigurerat fönster.
- `pf-blocker --monitor-slowloris-violations`  
  Importera IP-adresser markerade av Slowloris-detektorn till blocker-ledgern.
- `pf-blocker --monitor-ddos`  
  Importera IP-adresser markerade av DDoS-detektorn till blocker-ledgern.
- `pf-blocker --expire-blocks`  
  Ta bort gamla blockeringar från blockfilen och ledgern.
