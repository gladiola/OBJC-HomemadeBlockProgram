# OBJC-HomemadeBlockProgram — Kiswahili

Huu ni mpango wa mstari wa amri wa Objective-C kwa OpenBSD unaozuia IP za washambuliaji kutoka kumbukumbu na kuondoa vizuizi vya zamani.

Maelekezo ya matumizi yaliyotafsiriwa yapo hapa chini; README kamili ya Kiingereza imejumuishwa kwa usawa kamili.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Zuia IP zinazoonekana kwenye kumbukumbu za sshd "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Zuia IP zinazoonekana kwenye kumbukumbu za sshd "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Zuia IP zinazozidi kikomo cha ukiukaji wa allowlist ndani ya dirisha lililowekwa.
- `pf-blocker --monitor-slowloris-violations`  
  Ingiza IP zilizoalamishwa na kigunduzi cha Slowloris kwenye daftari la blocker.
- `pf-blocker --monitor-ddos`  
  Ingiza IP zilizoalamishwa na kigunduzi cha DDoS kwenye daftari la blocker.
- `pf-blocker --expire-blocks`  
  Ondoa vizuizi vya zamani kwenye faili ya vizuizi na daftari.
