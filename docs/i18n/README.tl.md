# OBJC-HomemadeBlockProgram — Tagalog

Ito ay Objective-C command-line program para sa OpenBSD na nagba-block ng attacker IP mula sa logs at nag-e-expire ng lumang blocks.

Nasa ibaba ang naisalokal na gabay sa paggamit; kasama ang buong English README para sa ganap na pagkakatugma.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  I-block ang mga IP na nakita sa sshd "Invalid user" log entries.
- `pf-blocker --monitor-disconnect`  
  I-block ang mga IP na nakita sa sshd "Received disconnect from" log entries.
- `pf-blocker --monitor-allowlist-violations`  
  I-block ang mga IP na lumampas sa web allowlist violation threshold sa nakatakdang window.
- `pf-blocker --monitor-slowloris-violations`  
  Ipasok sa blocker ledger ang mga IP na na-flag ng Slowloris detector.
- `pf-blocker --monitor-ddos`  
  Ipasok sa blocker ledger ang mga IP na na-flag ng DDoS detector.
- `pf-blocker --expire-blocks`  
  Alisin ang lumang blocks mula sa block file at ledger.
