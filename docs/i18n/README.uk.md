# OBJC-HomemadeBlockProgram — Українська

Це CLI-програма Objective-C для OpenBSD, яка блокує IP-адреси зловмисників із логів і видаляє застарілі блокування.

Нижче наведено локалізовані інструкції з використання; повний англомовний README додано для повної відповідності.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Блокує IP, що з’являються в записах sshd "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Блокує IP, що з’являються в записах sshd "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Блокує IP, які перевищують поріг порушень allowlist у заданому вікні.
- `pf-blocker --monitor-slowloris-violations`  
  Імпортує IP, позначені детектором Slowloris, у реєстр блокувань.
- `pf-blocker --monitor-ddos`  
  Імпортує IP, позначені детектором DDoS, у реєстр блокувань.
- `pf-blocker --expire-blocks`  
  Видаляє старі блокування з файла блокувань і реєстру.
