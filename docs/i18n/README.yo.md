# OBJC-HomemadeBlockProgram — Yorùbá

Eyi jẹ́ eto laini-àṣẹ Objective-C fún OpenBSD tí ó ń dènà IP àwọn olùkọlù láti inú log, tí ó sì ń parí ìdènà àtijọ́.

Àwọn ìtọnisọna lílo tí a túmọ̀ wà ní isalẹ; README Gẹ̀ẹ́sì pipe wà fún ìbámu pípé.

## Localized usage directions

- `pf-blocker --monitor-invalid-user`  
  Dènà IP tí a rí nínú ìforúkọsílẹ̀ sshd "Invalid user".
- `pf-blocker --monitor-disconnect`  
  Dènà IP tí a rí nínú ìforúkọsílẹ̀ sshd "Received disconnect from".
- `pf-blocker --monitor-allowlist-violations`  
  Dènà IP tí ó kọjá ààlà ìfojúsùn ìrúkọ allowlist nínú àkókò tí a ṣètò.
- `pf-blocker --monitor-slowloris-violations`  
  Gbé IP tí aṣàwárí Slowloris samisi wọ ledger blocker.
- `pf-blocker --monitor-ddos`  
  Gbé IP tí aṣàwárí DDoS samisi wọ ledger blocker.
- `pf-blocker --expire-blocks`  
  Yọ ìdènà àtijọ́ kúrò nínú fáìlì ìdènà àti ledger.
